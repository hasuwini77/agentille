...
- **Never let an agent push through context pressure.** Executors checkpoint at committable boundaries and self-report when their window fills (soft ~40% = no new scope; hard ~60% or any harness warning = checkpoint + hand off). The lead rotates in a fresh successor seeded from the checkpoint + context-pack slice — it never tells a filling agent to "just finish". See `team-mode.md` → "Context rotation".
...
- **Context discipline ladder (30/40/60).** The planner sizes each chunk to ≤ ~30% of an executor's window; at runtime an executor throttles (~40%: no new scope) and rotates out (~60%, or any harness warning) via checkpoint + successor. A half-full agent writes subtly worse code — rotation is cheaper than the review round-trip its bugs cost. See "Hard rules" above and `team-mode.md` → "Context rotation".
