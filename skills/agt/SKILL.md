...
11. **Tear down a team before declaring done (team mode only).** Run-end teardown is **mandatory and verified** — not advisory. The lead MUST shut down EVERY teammate spawned this run (by name, via `SendMessage`), verify each acknowledgment, and force-close any pane that persists in tmux before declaring done. An idle or spinning teammate at run end is an orphan to close — never "still needed". See `team-mode.md` → "Teardown" for the full checklist and the mid-run vs run-end distinction. (No-op in subagent/solo/workflow mode.)
