...
| ui-prototyper | **opus** | sonnet | The Prototype Blueprint sets the design direction the whole UI build (and the design-reviewer's score) follows — pay for taste, the same logic as the planner. Only dispatched on UI build work (`design`, or `feature`+hasUI), so it runs rarely. |
