...
**Dispatch with model aliases (`opus` / `sonnet` / `haiku`), not pinned IDs.** Aliases track the latest tier per Claude Code, so a rotated or unavailable exact ID can't hard-fail a dispatch on an OSS user's machine. The agent frontmatter carries the same aliases as a fallback default. The tier *identities* at time of writing — kept here as rationale, not as dispatch values — are: **opus** = the latest Opus (native vision, the planning/judgment/escalation ceiling), **sonnet** = the latest Sonnet (execution-grade reasoning, the default workhorse), **haiku** = the latest Haiku (classification/recap/scaffolding).
