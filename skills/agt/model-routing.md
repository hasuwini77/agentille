...
**Dispatch with model aliases (`opus` / `sonnet` / `haiku` / `fable`), not pinned IDs.** Aliases track the latest tier per Claude Code, so a rotated or unavailable exact ID can't hard-fail a dispatch on an OSS user's machine. The agent frontmatter carries the same aliases as a fallback default. The tier *identities* at time of writing — kept here as rationale, not as dispatch values — are: **opus** = Opus 4.8 (native vision, strongest reasoning), **sonnet** = Sonnet 4.6, **haiku** = Haiku 4.5, **fable** = Claude Fable 5 (escalation ceiling, ~2× opus cost — reserved for the largest/highest-stakes work).
...
| ui-prototyper | **opus** | sonnet | The Prototype Blueprint sets the design direction the whole UI build (and the design-reviewer's score) follows — pay for taste, the same logic as the planner. Only dispatched on UI build work (`design`, or `feature`+hasUI), so it runs rarely. → fable under `--fable` only. |
...
| design-reviewer | **opus** | opus | Never downgrade — opus (Opus 4.8) has native vision AND the strongest design judgment; design review is agentille's differentiator. The savings lever for design is **viewport scope** (capture only the viewports that matter — see `agentille-design-reviewer.md`), not the model. → fable under `--fable` only. |
